...
class Graph (object):
	...
	def hasCycle (self):
		...
		while N > 0:
			for vertex in graph.Vertices:
				lyst = graph.getNeighbors(vertex.label)
				if len(lyst) == 0:
	...
	def shortestPath(self,fromVertexLabel):
                vertices = self.getVertices()
                weights = self.adjMat[self.getIndex(fromVertexLabel)] # weights fromVertexLabel
                results = [] # destination, total path distance, parent vertex
                included = [] # list for marking vertices as visited, True or False
                # initialization phase
                for i in range(len(self.Vertices)):
                        included.append(True)
                for vertex in vertices:
                        if vertex == fromVertexLabel:
                                results.append([vertex,0,None]) # list distances directly visited from fromVertexLabel
                        for weight in weights:
                                if weight > 0 and weights.index(weight) == self.getIndex(vertex): # check if paths exist for fromVertexLabel
                        ...
                        if included[self.getIndex(vertex)] != False and vertex != fromVertexLabel :
                                ...
                                included[self.getIndex(vertex)] = False
                # computational phase
                while False in included: # exit once all vertices are visited
                        dists = []
                        for vertex in results:
                                if included[self.getIndex(vertex[0])] == False:
                                        dists.append(vertex[1])
                        minn = min(dists) # find minimum distances
                        for vertex in results:
                                if vertex[1] == minn:
                                        F = vertex 
                                        included[self.getIndex(F[0])] = True # mark vertex as visited
                                        weights = self.adjMat[self.getIndex(F[0])]  
                                        for vertex in results:
                                                for weight in weights:
                                                        if weight > 0 and weights.index(weight) == self.getIndex(vertex[0]) and (F[1] + weight < vertex[1]): # check if path exists and if less than previous paths
                                                                vertex[1] = F[1] + weight # add distances
                                                                vertex[2] = F[0] # mark parent vertex
                return results
	...
	def deleteVertex (self, vertexLabel):
		...
		for i in range (len(self.Vertices)):
			if self.Vertices[i].label != vertexLabel:
				newVertices.append(Vertex(self.Vertices[i].label))
		self.Vertices = newVertices
		
		# delete edges from adjacency matrix
		# delete row
		lyst = []
		for i in range (len(self.adjMat)):
			if i != idx:
				lyst.append(self.adjMat[i])
	...
	# Change all flags to false when you're done
		for i in range (len(self.Vertices)):
			self.Vertices[i].visited = False
...
def main():
	...
	if not (cities.hasCycle()):        # if graph has no cycles
	...
	result = cities.shortestPath(startVertex)
	lyst = []
	for i in result:
		if i[0] != startVertex:
			item = (startVertex + '->' + i[0] + ':' + ' ' + str(i[1]))
			if item not in lyst:
				print (item)
				lyst.append(item)
